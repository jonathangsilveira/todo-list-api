...
from src.api.tasks.request.todo_task_input import TodoTaskInput
from src.api.tasks.response.task import TaskResponse
...
async def get_all_todo_tasks(authorized_user: User = Depends(get_authorized_user),
    ...
    response = [TaskResponse.from_domain(task).model_dump(exclude_none=True) for task in todo_tasks]
    return JSONResponse(
...
@router.post(path="/task/upsert", status_code=status.HTTP_200_OK, response_model=TaskResponse)
async def upsert_todo_task(todo_task_input: TodoTaskInput,
                           authorized_user: User = Depends(get_authorized_user),
                           todo_tasks_service: TodoTasksService = Depends(get_todo_tasks_service)):
    new_todo_task = todo_task_input.to_domain()
...
@router.delete(path="/task/remove/{uuid}", status_code=status.HTTP_204_NO_CONTENT)
